...
// This variable controls how fast table descriptors are polled. A table descriptor
// must be read above the timestamp of any row that we'll emit.
...
func (tf *TableFeed) pollTableHistory(ctx context.Context) error {
	for {
		...
		case <-ctx.Done():
			return nil
		case <-time.After(ChangefeedPollInterval.Get(&tf.settings.SV)):
		}
	}
}
