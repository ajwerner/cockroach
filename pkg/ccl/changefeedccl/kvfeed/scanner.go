...
type scanner interface {
	Scan(ctx context.Context, sink *Buffer, cfg physicalConfig) error
}

func (p *scanRequestScanner) Scan(ctx context.Context, sink *Buffer, cfg physicalConfig) error {
	if log.V(2) {
		log.Infof(ctx, "performing scan on %v at %v withDiff %v", cfg.Spans, cfg.Timestamp, cfg.WithDiff)
	}
...
func (p *scanRequestScanner) exportSpan(
	ctx context.Context, span roachpb.Span, ts hlc.Timestamp, withDiff bool, sink *Buffer,
) error {
	...
	for remaining := span; ; {
		...
		if err := slurpScanResponse(ctx, remaining, res, ts, withDiff, sink); err != nil {
			return err
		}
