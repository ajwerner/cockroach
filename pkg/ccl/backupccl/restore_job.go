...
func createImportingTables(
	...
	for _, desc := range sqlDescs {
		...
		if dbDesc := desc.GetDatabase(); dbDesc != nil {
			if rewrite, ok := details.TableRewrites[dbDesc.ID]; ok {
				rewriteDesc := sqlbase.NewMutableDatabaseDescriptor(
					sqlbase.NewImmutableDatabaseDescriptor(&desc))
				rewriteDesc.SetID(rewrite.TableID)
				databases = append(databases, &rewriteDesc.ImmutableDatabaseDescriptor)
			}
		}
	}
	...
	if details.DescriptorCoverage == tree.AllDescriptors {
		databases = append(databases, sqlbase.NewImmutableDatabaseDescriptor(&sqlbase.Descriptor{
			DescriptorMeta: sqlbase.MakeInitialDescriptorMeta(sqlbase.ID(tempSystemDBID), restoreTempSystemDB),
