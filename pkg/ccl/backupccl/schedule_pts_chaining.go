...
func maybeUpdateSchedulePTSRecord(
	...
	return exec.InternalExecutorFactory.Txn(ctx, func(ctx context.Context, txn sqlutil.TransactionalExecutor) error {
		...
		sj, args, err := getScheduledBackupExecutionArgsFromSchedule(ctx, env, txn, scheduleID)
		if err != nil {
		...
		if args.BackupType == backuppb.ScheduledBackupExecutionArgs_FULL && args.DependentScheduleID == 0 {
			...
			return sj.Update(ctx, txn.InternalExecutor, txn.Txn)
		}
		...
		case backuppb.ScheduledBackupExecutionArgs_FULL:
			...
			if err := manageFullBackupPTSChaining(ctx, pts, txn, env, backupDetails, args); err != nil {
...
func manageFullBackupPTSChaining(
	...
	incSj, incArgs, err := getScheduledBackupExecutionArgsFromSchedule(
		ctx, env, txn, fullScheduleArgs.DependentScheduleID,
	)
	if err != nil {
	...
	return incSj.Update(ctx, txn.InternalExecutor, txn.Txn)
}
...
func getTargetProtectedByBackup(
	ctx context.Context, pts protectedts.Storage2, backupDetails jobspb.BackupDetails,
