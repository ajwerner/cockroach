...
type spanResolver struct {
	...
	oracleFactory leaseholder.OracleFactory
}
...
func NewSpanResolver(
	...
	nodeDesc roachpb.NodeDescriptor,
	rpcCtx *rpc.Context,
	policy leaseholder.ChoosingPolicy,
) SpanResolver {
	return &spanResolver{
		st:       st,
		nodeDesc: nodeDesc,
		oracleFactory: leaseholder.NewOracleFactory(policy, leaseholder.OracleConfig{
			Settings:         st,
			Gossip:           gossip,
			NodeDesc:         nodeDesc,
			RPCContext:       rpcCtx,
...
func (sr *spanResolver) NewSpanResolverIterator(txn *client.Txn) SpanResolverIterator {
	return &spanResolverIterator{
		...
		queryState: leaseholder.MakeOracleQueryState(),
	}
}
...
func (it *spanResolverIterator) ReplicaInfo(ctx context.Context) (kv.ReplicaInfo, error) {
	...
	repl, err := it.oracle.ChoosePreferredLeaseHolder(
