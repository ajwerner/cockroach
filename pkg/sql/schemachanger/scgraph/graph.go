...
// Graph is a graph whose nodes are scpb.TargetStates.
type Graph struct {
...
// Edge is represents a relationship between two TargetStates.
//
...
// OpEdge represents and edge changing the state of a target with an op.
type OpEdge struct {
	from, to *scpb.Node
	op       scop.Op
}
