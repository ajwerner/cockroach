...
func maybeExpandInvertedIndexes(sc *Schema, indexes []Index) ([]Index, error) {
	...
	for _, idx := range indexes {
		...
		if len(idx.Attrs) > 1 {
			return nil, errors.Errorf("inverted indexes may only reference a single slice attribute, got %v", idx.Attrs)
		}
		ord, err := sc.getOrdinal(idx.Attrs[0])
		if err != nil {
			return nil, errors.Wrap(err, "invalid index attribute")
		}
		et, ok := sc.sliceOrdinalTypes[ord]
		if !ok {
			return nil, errors.Errorf("inverted index references non-slice attribute %v", idx.Attrs[0])
		}
		if len(idx.Where) > 0 {
			return nil, errors.Errorf("inverted indexes are implicitly partial and cannot be further constrained")
		}
		if len(idx.Exists) > 0 {
			return nil, errors.Errorf("inverted indexes may not have existence constraints")
		}
		where := []IndexWhere{{
			Attr: Type,
			Eq:   et.typ,
...
func (t *Database) insert(v interface{}, es entityStore) (id int, err error) {
	if id, ok := t.entitySet.hash[v]; ok {
		return id, nil
	}
