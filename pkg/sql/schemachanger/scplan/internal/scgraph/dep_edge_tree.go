...
// the edge.
func (e *depEdges) insertOrUpdate(rule Rule, kind DepEdgeKind, from, to *screl.Node) error {
	k := makeEdgeKey(e.getTargetIdx, from, to)
	if got, ok := e.get(k); ok {
		return updateExistingDepEdge(rule, kind, got)
	}
	de := e.edgeAlloc.new(DepEdge{
		kind:  kind,
		from:  from,
		to:    to,
		rules: []Rule{rule},
	})
	e.fromTo.ReplaceOrInsert(e.entryAlloc.new(k, fromTo, de))
	e.toFrom.ReplaceOrInsert(e.entryAlloc.new(k, toFrom, de))
	return nil
}
...
func (et *depEdges) iterateFromNode(n *screl.Node, it DepEdgeIterator) (err error) {
	return iterateDepEdges(edgeKey{
		targets:  [2]targetIdx{0: et.getTargetIdx(n)},
		statuses: [2]uint8{0: uint8(n.CurrentStatus)},
	}, fromTo, et.fromTo, it)
}
...
func (et *depEdges) iterateToNode(n *screl.Node, it DepEdgeIterator) (err error) {
	return iterateDepEdges(edgeKey{
		targets:  [2]targetIdx{1: et.getTargetIdx(n)},
		statuses: [2]uint8{1: uint8(n.CurrentStatus)},
	}, toFrom, et.toFrom, it)
}

func iterateDepEdges(
	qk edgeKey, order edgeTreeOrder, t *btree.BTree, it DepEdgeIterator,
) (err error) {
...
func (e *depEdges) iterate(it DepEdgeIterator) (err error) {
	e.fromTo.Ascend(func(i btree.Item) bool {
		err = it(i.(*depEdgeTreeEntry).edge)
		return err == nil
	})
	return iterutil.Map(err)
