...
// AddIndexMutationMaybeWithBackfillingTempIndex adds an index mutation to desc.Mutations.
func (desc *Mutable) AddIndexMutationMaybeWithBackfillingTempIndex(
	ctx context.Context,
	idx *descpb.IndexDescriptor,
	...
	if !settings.Version.IsActive(ctx, clusterversion.MVCCIndexBackfiller) || !UseMVCCCompliantIndexCreation.Get(&settings.SV) {
		return desc.AddIndexMutation(idx, direction, defaultMutationStateForDirection(direction))
	}
...
func (desc *Mutable) AddIndexMutation(
	...
	if ok := (direction == descpb.DescriptorMutation_ADD &&
		(state == descpb.DescriptorMutation_BACKFILLING ||
			state == descpb.DescriptorMutation_DELETE_ONLY)) ||
		(direction == descpb.DescriptorMutation_DROP &&
			state == descpb.DescriptorMutation_DELETE_AND_WRITE_ONLY); !ok {
		return errors.AssertionFailedf("unexpected initial state %v for %v index mutation",
			state, direction)
	}
...
func defaultMutationStateForDirection(
