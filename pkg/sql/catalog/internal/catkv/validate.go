...
func NewCatalogReaderBackedValidationDereferencer(
	cr CatalogReader, txn *kv.Txn, dvmpMaybe DescriptorValidationModeProvider, locking bool,
...
func (c catalogReaderBackedDereferencer) DereferenceDescriptors(
	...
	read, err := c.cr.GetByIDs(ctx, c.txn, reqs, isDescriptorRequired, c.locking, catalog.Any)
	if err != nil {
		return nil, err
	}
