...
//go:generate genterraform -o terraform/main.tf
...
func (o *providerOpts) ConfigureClusterFlags(flags *pflag.FlagSet) {
	// Ensure that we can parse the default config that is embedded in the binary.
	if err := o.Config.Set(""); err != nil {
		panic(errors.Wrap(err, "failed to parse default config"))
	}
	flags.Var(&o.Config, ProviderName+"-config", "")
...
func (p *Provider) allRegions() (regions []string, err error) {
	...
	for _, z := range p.opts.Zones {
		az, ok := p.opts.Config.azByName[z]
		if !ok {
			return nil, fmt.Errorf("unknown availability zone %v, please provide a "+
				"correct value or update your config accordingly", z)
		}
		if _, have := byName[az.region.Region]; !have {
			byName[az.region.Region] = struct{}{}
			regions = append(regions, az.region.Region)
		}
	}
	return regions, nil
}
