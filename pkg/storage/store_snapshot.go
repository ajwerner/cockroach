...
func (s *Store) canApplySnapshotLocked(
	...
	// is imminent.
	if !existingDestroyStatus.IsAlive() {
...
func (s *Store) tryAcceptSnapshotData(
	...
	case destroyReasonAlive:
		...
		log.Infof(ctx, "encountered snapshot for r%s/%d with replica id %d %v, destroying",
			existingRepl.RangeID, snapReplicaDesc.ReplicaID, existingRepl.mu.replicaID)
		// We're not holding the raftMu so we can't synchronously delete the replica
		// right here. Instead we'll run an async task to do the removal.
	...
	log.Fatalf(ctx, "unknown destroyStatus reason %v", destroyStatus)
