...
func (bq *baseQueue) pop() replicaInQueue {
	...
	for {
		...
		if repl != nil {
			...
			if item.replicaID != repl.ReplicaID() {
				log.Infof(context.TODO(), "not processing replica for range %d added with replica id %d that now has id %d",
					item.value, item.replicaID, repl.ReplicaID())
			}
		}
