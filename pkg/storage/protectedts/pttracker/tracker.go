...
// TODO(ajwerner): hook up periodic fetching to gossip so that just
...
// Read locks would help with this contention problem.

// TODO(ajwerner): Add metrics.
// TODO(ajwerner): Add memory accounting.
...
type Tracker struct {
	...
	mu        struct {
		...
		// An interval tree would do but it might be more complicated than it's
		// worth.
	}
}
...
func New(
	settings *cluster.Settings, db *client.DB, storage protectedts.Storage,
) protectedts.Tracker {
