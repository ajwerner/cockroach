...
type groupIDChunk struct {
	...
	buf    [groupIDChunkSize]GroupID
	rd, wr int
}

func (c *groupIDChunk) PushBack(id GroupID) bool {
	if c.WriteCap() == 0 {
		return false
	}
	c.buf[c.wr] = id
...
func (c *groupIDChunk) PopFront() (GroupID, bool) {
	if c.Len() == 0 {
		return 0, false
	}
	id := c.buf[c.rd]
...
// groupIDQueue is a chunked queue of range IDs. Instead of a separate list
// element for every range ID, it uses a groupIDChunk to hold many range IDs,
...
// reallocation).
type groupIDQueue struct {
	chunks list.List
	len    int
}

func (q *groupIDQueue) PushBack(id GroupID) {
...
func (q *groupIDQueue) PopFront() (GroupID, bool) {
	if q.len == 0 {
		return 0, false
	}
...
func (q *groupIDQueue) Len() int {
	return q.len
}

func (q *groupIDQueue) back() *groupIDChunk {
